...
<workflow>
  ...
  <step n="5" goal="Implement task following red-green-refactor cycle">
    ...
    <action>Before writing implementation code, identify every external library, framework, SDK, API, or CLI tool the task depends on</action>
    <action if="the task uses any external library/framework/SDK/API/CLI">Consult the context7 MCP for current documentation: call `mcp__context7__resolve-library-id` to resolve each library, then `mcp__context7__query-docs` to fetch up-to-date API syntax, configuration, and usage patterns BEFORE writing code. Do NOT rely on memory for library specifics — training data may be stale.</action>
    <action if="context7 MCP is unavailable or returns no match for a library">Record this in Dev Agent Record → Debug Log and proceed with best available knowledge</action>
    <critical>For any code touching an external library/framework/SDK/API, ground the implementation in context7 docs rather than recalled API shapes</critical>

    <!-- GREEN PHASE -->
    <action>Implement MINIMAL code to make tests pass, applying the context7-sourced documentation</action>
